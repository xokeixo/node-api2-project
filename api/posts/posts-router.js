...
const Adopter = require('./posts-model');
...
router.get('/', (req, res) => {
  ...
  router.get('/:id/posts', (req, res) => {
    Adopter.findPosts(req.params.id)
      .then(posts => {
        if (posts.length > 0) {
          res.status(200).json(posts);
        } else {
          res.status(404).json({ message: 'No posts for this adopter' });
        }
      })
      .catch(error => {
        console.log(error);
        res.status(500).json({
          message: 'Error retrieving the posts for this adopter',
        });
      });
  });
